-- Migration: 020_search_rank_featured
-- Description: Add is_featured (high priority) to search ranking.
-- Featured profiles rank above normal profiles but below the low-priority sort.
...
BEGIN
    ...
    ORDER BY
        ...
        -- 2. High priority (featured) profiles go to the top
        COALESCE(p.is_featured, false) DESC,
        -- 3. Verified first
        p.is_verified DESC,
        -- 4. Contact info + web presence — more ways to reach = ranked higher
        (
            CASE WHEN p.contact_email IS NOT NULL THEN 0 ELSE 3 END +
            CASE WHEN p.contact_phone IS NOT NULL THEN 0 ELSE 1 END +
            CASE WHEN p.website IS NOT NULL THEN 0 ELSE 1 END +
            CASE WHEN p.portfolio_url IS NOT NULL THEN 0 ELSE 1 END
        ) ASC,
        -- 5. Exact name match first when searching by text
        CASE
            WHEN search_text IS NOT NULL AND search_text != '' AND
                 p.name ILIKE '%' || search_text || '%' THEN 0
            ELSE 1
        END ASC,
        -- 5. Primary role match before secondary role match
        CASE
            ...
            ELSE 0
        END ASC,
        -- 6. Text relevance when searching
        CASE
            ...
            ELSE 0
        END DESC,
        -- 7. Quality score: credits + experience + completeness
        (
