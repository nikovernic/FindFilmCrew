-- Migration: 021_fix_search_ranking_order
-- Description: Reorder search ranking so primary role match is the most important
-- signal (after low-priority/featured/verified), then contact info as a secondary signal.
...
BEGIN
    ...
    ORDER BY
        ...
        -- 2. Featured profiles go to the top
        COALESCE(p.is_featured, false) DESC,
        -- 3. Verified first
        p.is_verified DESC,
        -- 4. Primary role match is the most important ranking signal
        CASE
            ...
            ELSE 0
        END ASC,
        -- 5. Exact name match when searching by text
        CASE
            WHEN search_text IS NOT NULL AND search_text != '' AND
                 p.name ILIKE '%' || search_text || '%' THEN 0
            ELSE 1
        END ASC,
        -- 6. People with contact info ranked higher
        (
        ...
        -- 7. Text relevance when searching
        CASE
            ...
            ELSE 0
        END DESC,
        -- 8. Quality score: credits + experience + completeness
        (
